...
namespace Microsoft.Azure.Commands.SecurityCenter.Cmdlets.RegulatoryCompliance
{
    ...
    public class GetRegulatoryComplianceControl: SecurityCenterCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ParameterSetNames.ParentObjectScope, Mandatory = true, HelpMessage = ParameterHelpMessages.StandardObject)]
        [ValidateNotNullOrEmpty]
        public PSSecurityRegulatoryComplianceStandard StandardObject { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            switch (ParameterSetName)
            {
                ...
                case ParameterSetNames.ParentObjectScope:
                    ...
                    {
                        var regulatoryComplianceControl = SecurityCenterClient.RegulatoryComplianceControls.GetWithHttpMessagesAsync(
                            StandardObject.Name, Name).GetAwaiter().GetResult().Body;
                        WriteObject(regulatoryComplianceControl.ConvertToPSType(), enumerateCollection: false);
                        break;
                    }
                    else
                    {
                        var regulatoryComplianceControls = SecurityCenterClient.RegulatoryComplianceControls.ListWithHttpMessagesAsync(
                            StandardObject.Name).GetAwaiter().GetResult().Body;
                        WriteObject(regulatoryComplianceControls.ConvertToPSType(), enumerateCollection: true);
                        break;
                    }

                case ParameterSetNames.ResourceId:
                    var regulatoryComplianceControl1 = SecurityCenterClient.RegulatoryComplianceControls.GetWithHttpMessagesAsync(
                        AzureIdUtilities.GetRegulatoryStandardName(ResourceId), 
                        AzureIdUtilities.GetRegulatoryStandardControlName(ResourceId))
                        .GetAwaiter().GetResult().Body;
                    WriteObject(regulatoryComplianceControl1.ConvertToPSType(), enumerateCollection: false);
                    break;
                default:
