...
{
    [Cmdlet(VerbsCommon.Get, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "SecurityComplainceAssessment", DefaultParameterSetName = ParameterSetNames.SubscriptionLevelResource), OutputType(typeof(PSSecurityRegulatoryComplianceAssessment))]
    ...
    public class GetRegulatoryComplianceAssessment: SecurityCenterCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ParameterSetNames.ParentObjectScope, Mandatory = false, HelpMessage = ParameterHelpMessages.ResourceName)]
        ...
        [Parameter(ParameterSetName = ParameterSetNames.ParentObjectScope, Mandatory = true, HelpMessage = ParameterHelpMessages.ControlObject)]
        [ValidateNotNullOrEmpty]
        public PSSecurityRegulatoryComplianceControl ControlObject { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            switch (ParameterSetName)
            {
                ...
                case ParameterSetNames.ParentObjectScope:
                    if (!string.IsNullOrEmpty(Name))
                    {
                        var regulatoryComplianceAssessment = SecurityCenterClient.RegulatoryComplianceAssessments.GetWithHttpMessagesAsync(
                            AzureIdUtilities.GetRegulatoryStandardControlName(ControlObject.Id), ControlObject.Name, Name).GetAwaiter().GetResult().Body;
                        WriteObject(regulatoryComplianceAssessment.ConvertToPSType(), enumerateCollection: false);
                        break;
                    }
                    else
                    {
                        var regulatoryComplianceAssessments = SecurityCenterClient.RegulatoryComplianceAssessments.ListWithHttpMessagesAsync(
                            AzureIdUtilities.GetRegulatoryStandardControlName(ControlObject.Id), ControlObject.Name).GetAwaiter().GetResult().Body;
                        WriteObject(regulatoryComplianceAssessments.ConvertToPSType(), enumerateCollection: true);
                        break;
                    }

                case ParameterSetNames.ResourceId:
                    var regulatoryComplianceAssessment1 = SecurityCenterClient.RegulatoryComplianceAssessments.GetWithHttpMessagesAsync(
                        AzureIdUtilities.GetRegulatoryStandardControlName(ResourceId), 
                        AzureIdUtilities.GetRegulatoryStandardControlName(ResourceId), 
                        AzureIdUtilities.GetRegulatoryStandardAssessmentName(ResourceId))
                        .GetAwaiter().GetResult().Body;
                    WriteObject(regulatoryComplianceAssessment1.ConvertToPSType(), enumerateCollection: false);
                    break;
                default:
